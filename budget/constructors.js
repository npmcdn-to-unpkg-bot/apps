function Budget(deposits,transactions,recurringTransactions,history) {
	...
	this.log = function(action, store, date) {
		var histo = new History(action, store, date);
		this.history.unshift(histo);
		//this.transactions.push(trans);
	};
	...
	function History(action, store, date) {
		this.action = action;
		this.store = store;
		this.date = date;
		this.String = this.action+" Transaction with "+this.store+" at "+this.date;
